...
- [x] Audit caches keyed by repo id, path, owner/repo, or active runtime and add host/source identity where needed.
- [x] Ensure request ownership is explicit for provider fetches, provider mutations, file-system calls, git calls, terminal calls, and automation runs.
...
- [x] 2026-06-13: Completed the cache/request ownership audit by classifying the remaining active-runtime uses and verifying that provider fetches/mutations, filesystem/git/terminal/worktree requests, pending workspace creation, task drawer replay, source-control review actions, and automation run/save paths carry explicit source or run-host identity. Remaining durable headless remote-server scheduling and practical version-skew matrix coverage stay tracked as separate rows.
- [x] 2026-06-13: Routed Automations list/create/update/delete/run-now operations through the owning remote server when the selected automation host is a runtime host, while keeping local/SSH automations on desktop IPC. The main automation service now only accepts `remote_host_service` schedules in `--serve` mode, so desktop clients cannot accidentally dispatch mirrored remote-owned automations. Verified with focused automation host-client, service, availability, runtime RPC, and runtime automation tests plus targeted oxlint, full `pnpm run typecheck`, and `git diff --check`. The durable remote-server row remains partial because fully headless `--serve` still has no renderer-independent automation dispatcher.
