...
import {
  ...
  remoteTrackingRefExistsWithCache
} from './remote-tracking-ref-cache'
...
async function readOrProbeEffectiveUpstreamStatus(
  ...
  // Why: source-control mount and the root git poll can overlap. Coalesce the
  // richer upstream probe so a stable missing origin ref only fails once.
...
async function probeEffectiveUpstreamStatus(
  ...
  const status = await getEffectiveGitUpstreamStatus(async (args) => {
    if (args[0] === 'rev-parse' && args.includes(`refs/remotes/origin/${branchName}`)) {
      probedSameNameOriginRef = true
      const ref = args.find((arg) => arg.startsWith('refs/remotes/origin/'))
      if (ref) {
        if (await remoteTrackingRefExistsWithCache(worktreePath, ref)) {
          return { stdout: '' }
        }
        throw Object.assign(new Error(`missing remote branch: ${ref}`), { code: 1, stderr: '' })
      }
    }
